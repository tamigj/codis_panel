...
n_commas = str_count(filters,",") + 1
filters = unlist(strsplit(filters, ",", n_commas))
...
for (n_snps in n_snps_list){
  for (n_snp_rep in 1:n_snp_reps){
    ...
    snplist_id = str_interp("${experiment}_${filter}_${n_snps}_${n_snp_rep}")
    n_total_snps = 0
...
summary_df = main_df %>%
  ...
  group_by(filter, n_snps_per_str) %>%
  ...
  select(experiment, filter, fraction,
